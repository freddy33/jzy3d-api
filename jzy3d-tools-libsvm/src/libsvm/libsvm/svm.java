...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int _v=h.len; h.len=len; len=_v;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] _v=head[i].data; head[i].data=head[j].data; head[j].data=_v;} while(false);
		do {int _v=head[i].len; head[i].len=head[j].len; head[j].len=_v;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int _v=i; i=j; j=_v;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float _v=h.data[i]; h.data[i]=h.data[j]; h.data[j]=_v;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
    ...
    void swap_index(int i, int j)
	{
		do {svm_node[] _v=x[i]; x[i]=x[j]; x[j]=_v;} while(false);
		if(x_square != null) do {double _v=x_square[i]; x_square[i]=x_square[j]; x_square[j]=_v;} while(false);
	}
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte _v=y[i]; y[i]=y[j]; y[j]=_v;} while(false);
		do {double _v=G[i]; G[i]=G[j]; G[j]=_v;} while(false);
		do {byte _v=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=_v;} while(false);
		do {double _v=alpha[i]; alpha[i]=alpha[j]; alpha[j]=_v;} while(false);
		do {double _v=p[i]; p[i]=p[j]; p[j]=_v;} while(false);
		do {int _v=active_set[i]; active_set[i]=active_set[j]; active_set[j]=_v;} while(false);
		do {double _v=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=_v;} while(false);
	}
...
class SVC_Q extends Kernel
{
    ...
    void swap_index(int i, int j)
	{
		...
		do {byte _v=y[i]; y[i]=y[j]; y[j]=_v;} while(false);
		do {float _v=QD[i]; QD[i]=QD[j]; QD[j]=_v;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
    ...
    void swap_index(int i, int j)
	{
		do {byte _v=sign[i]; sign[i]=sign[j]; sign[j]=_v;} while(false);
		do {int _v=index[i]; index[i]=index[j]; index[j]=_v;} while(false);
		do {float _v=QD[i]; QD[i]=QD[j]; QD[j]=_v;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int _v=perm[i]; perm[i]=perm[j]; perm[j]=_v;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int _v=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=_v;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
